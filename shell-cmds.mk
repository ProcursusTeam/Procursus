...
shell-cmds: shell-cmds-setup
	mkdir -p $(BUILD_STAGE)/shell-cmds/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/bin
	...
	cp -a $(MACOSX_SYSROOT)/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/include/sys/user.h $(BUILD_WORK)/shell-cmds/include/sys
	cp -a $(BUILD_BASE)/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/include/unistd.h $(BUILD_WORK)/shell-cmds/include

	cd $(BUILD_WORK)/shell-cmds; \
	$(CC) -arch $(MEMO_ARCH) -isysroot $(TARGET_SYSROOT) $(PLATFORM_VERSION_MIN) -o $(BUILD_STAGE)/shell-cmds/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/bin/hexdump hexdump/{conv,display,hexdump,hexsyntax,odsyntax,parse}.c -D'__FBSDID(x)=' -D__DARWIN_C_LEVEL=200112L; \
	for bin in killall renice script time which getopt what; do \
    	$(CC) -arch $(MEMO_ARCH) -isysroot $(TARGET_SYSROOT) $(PLATFORM_VERSION_MIN) -isystem include -o $(BUILD_STAGE)/shell-cmds/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/bin/$$bin $$bin/*.c -D'__FBSDID(x)=' -save-temps; \
	done
