...
apr-util-setup: setup
	wget -nc -P$(BUILD_SOURCE) https://archive.apache.org/dist/apr/apr-util-$(APR_UTIL_VERSION).tar.bz2
...
apr-util: apr-util-setup apr expat libgdbm openssl unixodbc
	cd $(BUILD_WORK)/apr-util && autoreconf -fi && ./configure -C \
		...
		--with-crypto \
		--with-gdbm="$(BUILD_BASE)$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)" \
		...
		APR_LIBS="-L$(BUILD_BASE)$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/lib -R$(BUILD_BASE)$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/lib -lapr-1" \
		ac_cv_lib_crypto_EVP_CIPHER_CTX_new=yes \
		ac_cv_lib_ssl_SSL_accept=yes
	sed -i -e "s|^CC=.*$$|CC=$(CC)|" \
		-e "s|^CFLAGS=.*$$|CFLAGS=$(CFLAGS) -I$(BUILD_BASE)$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/apr-1|" \
		-e "s|^CPPFLAGS=.*$$|CPPFLAGS=$(CPPFLAGS) -I$(BUILD_BASE)$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/apr-1|" \
...
apr-util-package: apr-util-stage
	...
	rm -rf $(BUILD_DIST)/libaprutil1{,-dbd-sqlite3,-dev}
	mkdir -p $(BUILD_DIST)/libaprutil1{,-dbd-sqlite3,-dev}/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/lib/apr-util-1
