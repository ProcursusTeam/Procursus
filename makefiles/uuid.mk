...
uuid-package: uuid-stage
	...
	mkdir -p $(BUILD_DIST)/uuid/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/{bin,share/man/man1} \
		$(BUILD_DIST)/libuuid-dev/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/{bin,share/man/man1,lib/$(MULTIARCH_TARGET)} \
		$(BUILD_DIST)/libuuid16/$(MEMO_LIBDIR)
	...
	cp -a $(BUILD_STAGE)/uuid/$(MEMO_LIBDIR)/libuuid.16.dylib $(BUILD_DIST)/libuuid16/$(MEMO_LIBDIR)

	# uuid.mk Prep libuuid-dev
	cp -a $(BUILD_STAGE)/uuid/$(MEMO_LIBDIR)/!(libuuid.16.dylib) $(BUILD_DIST)/libuuid-dev/$(MEMO_LIBDIR)
