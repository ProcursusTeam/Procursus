...
GMP_CFBASE_VERSION   := 6.2.1
DEB_GMP_CFBASE_V     ?= $(GMP_CFBASE_VERSION)-3

libgmp10_CFbase-setup: setup
	$(call DOWNLOAD_FILES,$(BUILD_SOURCE),https://gmplib.org/download/gmp/gmp-$(GMP_CFBASE_VERSION).tar.xz{$(comma).sig})
	$(call PGP_VERIFY,gmp-$(GMP_CFBASE_VERSION).tar.xz)
	$(call EXTRACT_TAR,gmp-$(GMP_CFBASE_VERSION).tar.xz,gmp-$(GMP_CFBASE_VERSION),libgmp10)

ifneq ($(wildcard $(BUILD_WORK)/libgmp10/.build_complete),)
libgmp10_CFbase:
	@echo "Using previously built libgmp10."
else
libgmp10_CFbase: libgmp10_CFbase-setup
...
#	https://github.com/ProcursusTeam/Procursus/issues/750
	cd $(BUILD_WORK)/libgmp10 && ./configure -C \
		...
		CXXFLAGS='-stdlib=libstdc++ -D__GMPXX_USE_CXX11=0'
...
.PHONY: libgmp10_CFbase libgmp10_CFbase-package
