...
aptitude-setup: setup
	wget2 -q -nc -P $(BUILD_SOURCE) http://deb.debian.org/debian/pool/main/a/aptitude/aptitude_$(APTITUDE_VERSION).orig.tar.xz
...
aptitude: aptitude-setup ncurses libboost xapian cwidget apt googletest libsigcplusplus
	...
	cd $(BUILD_WORK)/aptitude && autoreconf -fi
	cd $(BUILD_WORK)/aptitude && ./configure -C \
		...
		--with-package-state-loc="$(MEMO_PREFIX)/var/lib/aptitude" \
		--with-lock-loc="$(MEMO_PREFIX)/var/lock/aptitude" \
	...
	mkdir -p $(BUILD_STAGE)/aptitude/$(MEMO_PREFIX)/var/lock/
	$(call AFTER_BUILD)
endif
