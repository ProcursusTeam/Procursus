...
berkeleydb-package: berkeleydb-stage
	...
	mkdir -p $(BUILD_DIST)/db18.1-util/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX) \
		$(BUILD_DIST)/libdb18.1{,++,-stl}{/$(MEMO_LIBDIR),-dev/{$(MEMO_LIBDIR),$(MEMO_INCDIR)/db181}}
	...
	cp -a $(BUILD_STAGE)/berkeleydb/$(MEMO_LIBDIR)/libdb-18{,.1}.dylib $(BUILD_DIST)/libdb18.1/$(MEMO_LIBDIR)

	# berkeleydb.mk Prep libdb18.1++
	cp -a $(BUILD_STAGE)/berkeleydb/$(MEMO_LIBDIR)/libdb_cxx-18{,.1}.dylib $(BUILD_DIST)/libdb18.1++/$(MEMO_LIBDIR)

	# berkeleydb.mk Prep libdb18.1-stl
	cp -a $(BUILD_STAGE)/berkeleydb/$(MEMO_LIBDIR)/libdb_stl-18{,.1}.dylib $(BUILD_DIST)/libdb18.1-stl/$(MEMO_LIBDIR)

	# berkeleydb.mk Prep libdb18.1-dev
	cp -a $(BUILD_STAGE)/berkeleydb/$(MEMO_LIBDIR)/libdb{-18.1.a,.dylib} $(BUILD_DIST)/libdb18.1-dev/$(MEMO_LIBDIR)
	cp -a $(BUILD_STAGE)/berkeleydb/$(MEMO_INCDIR)/db181/db{,_185}.h $(BUILD_DIST)/libdb18.1-dev/$(MEMO_INCDIR)/db181

	# berkeleydb.mk Prep libdb18.1++-dev
	cp -a $(BUILD_STAGE)/berkeleydb/$(MEMO_LIBDIR)/libdb_cxx{-18.1.a,.dylib} $(BUILD_DIST)/libdb18.1++-dev/$(MEMO_LIBDIR)
	cp -a $(BUILD_STAGE)/berkeleydb/$(MEMO_INCDIR)/db181/db_cxx.h $(BUILD_DIST)/libdb18.1++-dev/$(MEMO_INCDIR)/db181

	# berkeleydb.mk Prep libdb18.1-stl-dev
	cp -a $(BUILD_STAGE)/berkeleydb/$(MEMO_LIBDIR)/libdb_stl{-18.1.a,.dylib} $(BUILD_DIST)/libdb18.1-stl-dev/$(MEMO_LIBDIR)
	cp -a $(BUILD_STAGE)/berkeleydb/$(MEMO_INCDIR)/db181/dbstl*.h $(BUILD_DIST)/libdb18.1-stl-dev/$(MEMO_INCDIR)/db181
