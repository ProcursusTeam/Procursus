...
OPENSSH_CFBASE_VERSION := 9.2p1

openssh_CFbase-setup: setup
	$(call DOWNLOAD_FILES,$(BUILD_SOURCE),https://cdn.openbsd.org/pub/OpenBSD/OpenSSH/portable/openssh-$(OPENSSH_CFBASE_VERSION).tar.gz{$(comma).asc})
	$(call PGP_VERIFY,openssh-$(OPENSSH_CFBASE_VERSION).tar.gz,asc)
	$(call EXTRACT_TAR,openssh-$(OPENSSH_CFBASE_VERSION).tar.gz,openssh-$(OPENSSH_CFBASE_VERSION),openssh)
...
ifeq (,$(findstring darwin,$(MEMO_TARGET)))
ifeq (,$(findstring ramdisk,$(MEMO_TARGET)))
openssh_CFbase: openssh-setup openssl libxcrypt openpam libmd
else #(,$(findstring ramdisk,$(MEMO_TARGET)))
openssh_CFbase: openssh-setup openssl libmd
endif #(,$(findstring ramdisk,$(MEMO_TARGET)))
...
ifeq (,$(findstring ramdisk,$(MEMO_TARGET)))
	cd $(BUILD_WORK)/openssh && ./configure -C \
		...
		LIBS="-L/usr/lib/gcc/arm-apple-darwin9/4.0.1/v6/ -lgcc" \
...
openssh_CFbase-package:: DEB_OPENSSH_V ?= $(OPENSSH_CFBASE_VERSION)
openssh_CFbase-package: openssh-stage
...
.PHONY: openssh_CFbase openssh_CFbase-package
