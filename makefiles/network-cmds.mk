...
network-cmds: network-cmds-setup libpcap
	...
	$(CC) $(CFLAGS) -c network_cmds_lib/network_cmds_lib.c -Inetwork_cmds_lib -o network_cmds_lib.o; \
	...
	for bin in cfilutil dnctl frame_delay pktapctl pktmnglr; do \
		case $$bin in \
			ecnprobe) NETWORK_CMDS_CFLAGS="network_cmds_lib.o -Inetwork_cmds_lib";; \
		esac; \
		echo $$bin; \
		$(CC) -Iinclude -DPRIVATE=1 $$NETWORK_CMDS_CFLAGS $(CFLAGS) $(LDFLAGS) $${bin}/*.c -o $${bin}/$${bin}; \
	done
	cp -a $(BUILD_WORK)/network-cmds/kdumpd $(BUILD_STAGE)/network-cmds/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/libexec
	cp -a $(BUILD_WORK)/network-cmds/{arp,dnctl/dnctl,frame_delay/frame_delay,ifconfig,ip6addrctl,kdumpd,mtest,ndp,ping,ping6,pktapctl/pktapctl,route,rtsol,traceroute6,traceroute,netstat} $(BUILD_STAGE)/network-cmds/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/sbin
	...
	cp -a $(BUILD_WORK)/network-cmds/{dnctl,frame_delay,pktapctl,{arp,ifconfig,ip6addrctl,kdumpd,mtest,ndp,ping,ping6,route,rtsol,traceroute6,traceroute}.tproj}/*.8 $(BUILD_STAGE)/network-cmds/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/share/man/man8
	...
	for bin in ifconfig ip6addrctl netstat ping ping6 route rtsol; do \
		$(LN_SR) $(BUILD_STAGE)/network-cmds/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/sbin/$$bin $(BUILD_STAGE)/network-cmds/$(MEMO_PREFIX)/sbin; \
	done
	$(LN_SR) $(BUILD_STAGE)/network-cmds/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/sbin/ping $(BUILD_STAGE)/network-cmds/$(MEMO_PREFIX)/bin
	$(LN_SR) $(BUILD_STAGE)/network-cmds/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/sbin/ping6 $(BUILD_STAGE)/network-cmds/$(MEMO_PREFIX)/bin
	$(LN_SR) $(BUILD_STAGE)/network-cmds/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/sbin/ping $(BUILD_STAGE)/network-cmds/$(MEMO_PREFIX)/sbin
	$(LN_SR) $(BUILD_STAGE)/network-cmds/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/sbin/ping6 $(BUILD_STAGE)/network-cmds/$(MEMO_PREFIX)/sbin
