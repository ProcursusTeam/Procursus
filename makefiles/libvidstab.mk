...
libvidstab-package: libvidstab-stage
	...
	rm -rf $(BUILD_DIST)/libvidstab{1.2,-dev}
	mkdir -p $(BUILD_DIST)/libvidstab{1.2,-dev}/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/lib

	# libvidstab.mk Prep libvidstab1.2
	cp -a $(BUILD_STAGE)/libvidstab/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/lib/libvidstab.1.2.dylib $(BUILD_DIST)/libvidstab1.2/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/lib

	# libvidstab.mk Prep libvidstab-dev
	cp -a $(BUILD_STAGE)/libvidstab/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/lib/!(libvidstab.1.2.dylib) $(BUILD_DIST)/libvidstab-dev/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/lib
	cp -a $(BUILD_STAGE)/libvidstab/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/include $(BUILD_DIST)/libvidstab-dev/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)

	# libvidstab.mk Sign
	$(call SIGN,libvidstab1.2,general.xml)

	# libvidstab.mk Make .debs
	$(call PACK,libvidstab1.2,DEB_LIBVIDSTAB_V)
