...
brotli: brotli-setup
	...
	+$(MAKE) -C $(BUILD_WORK)/brotli install \
		DESTDIR="$(BUILD_BASE)"
	for lib in $(BUILD_STAGE)/brotli/$(MEMO_LIBDIR)/libbrotli{common,dec,enc}-static.a; do \
		if [ -f $$lib ]; then \
...
brotli-package: brotli-stage
	...
	mkdir -p $(BUILD_DIST)/brotli/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/{bin,share/man/man1} \
			$(BUILD_DIST)/libbrotli-dev/{$(MEMO_INCDIR)/brotli,$(MEMO_LIBDIR)/pkgconfig} \
			$(BUILD_DIST)/libbrotli1/$(MEMO_LIBDIR)
	...
	cp -a $(BUILD_STAGE)/brotli/$(MEMO_INCDIR)/brotli/{decode,encode,port,types}.h $(BUILD_DIST)/libbrotli-dev/$(MEMO_INCDIR)/brotli
	cp -a $(BUILD_STAGE)/brotli/$(MEMO_LIBDIR)/libbrotli{common,dec,enc}.{a,dylib} $(BUILD_DIST)/libbrotli-dev/$(MEMO_LIBDIR)
	cp -a $(BUILD_STAGE)/brotli/$(MEMO_LIBDIR)/pkgconfig/libbrotli{common,dec,enc}.pc $(BUILD_DIST)/libbrotli-dev/$(MEMO_LIBDIR)/pkgconfig

	# brotli.mk Prep libbrotli1
	cp -a $(BUILD_STAGE)/brotli/$(MEMO_LIBDIR)/libbrotli{common,dec,enc}.1*.dylib $(BUILD_DIST)/libbrotli1/$(MEMO_LIBDIR)
