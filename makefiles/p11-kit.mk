...
p11-kit-package: p11-kit-stage
	...
	mkdir -p $(BUILD_DIST)/p11-kit/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/share \
		$(BUILD_DIST)/p11-kit-modules/$(MEMO_LIBDIR) \
		$(BUILD_DIST)/libp11-kit0/$(MEMO_LIBDIR) \
		$(BUILD_DIST)/libp11-kit-dev/{$(MEMO_LIBDIR),$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/share}
	...
	cp -a $(BUILD_STAGE)/p11-kit/$(MEMO_LIBDIR)/pkcs11 $(BUILD_DIST)/p11-kit-modules/$(MEMO_LIBDIR)

	# p11-kit.mk Prep libp11-kit0
	cp -a $(BUILD_STAGE)/p11-kit/$(MEMO_LIBDIR)/libp11-kit.0.dylib $(BUILD_DIST)/libp11-kit0/$(MEMO_LIBDIR)
	cp -a $(BUILD_STAGE)/p11-kit/$(MEMO_PREFIX)/etc $(BUILD_DIST)/libp11-kit0/$(MEMO_PREFIX)

	# p11-kit.mk Prep libp11-kit-dev
	cp -a $(BUILD_STAGE)/p11-kit/$(MEMO_LIBDIR)/!(libp11-kit.0.dylib|pkcs11) $(BUILD_DIST)/libp11-kit-dev/$(MEMO_LIBDIR)
