...
xorg-server: xorg-server-setup libmd libx11 libxau libxmu xorgproto font-util libpixman libpng16 mesa libxfont2 libxkbfile libxdamage libxt libxpm libxaw libxres libxext xcb-util xcb-util-renderutil xcb-util-image xcb-util-wm xcb-util-keysyms libxdmcp libxfixes libxi libxrender libxtst libxxf86dga libxcomposite libxv libxvmc libxcvt libxss
	...
	+$(INSTALL) -m644 $(BUILD_WORK)/xorg-server/xorg-server-$(XORG-SERVER_VERSION).tar.xz $(BUILD_STAGE)/xorg-server/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/src
...
xorg-server-package: xorg-server-stage
	...
	mkdir -p $(BUILD_DIST)/xserver-xorg-core/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/lib \
		...
		$(BUILD_DIST)/xorg-server-source/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)
	...
	cp -a $(BUILD_STAGE)/xorg-server/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/share/man/man1/Xserver.1$(MEMO_MANPAGE_SUFFIX) $(BUILD_DIST)/xserver-common/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/share/man/man1
	...
	# xorg-server.mk Prep xorg-server-source
	cp -a $(BUILD_STAGE)/xorg-server/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)/src $(BUILD_DIST)/xorg-server-source/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX)
	...
	$(call PACK,xorg-server-source,DEB_XORG-SERVER_V)
