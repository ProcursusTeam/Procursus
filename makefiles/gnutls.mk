...
gnutls: gnutls-setup readline gettext libgcrypt libgmp10 libidn2 libunistring nettle p11-kit
	...
	cd $(BUILD_WORK)/gnutls && ./configure -C \
		...
		P11_KIT_CFLAGS=-I$(BUILD_BASE)$(MEMO_INCDIR)/p11-kit-1
...
gnutls-package: gnutls-stage
	...
	mkdir -p $(BUILD_DIST)/gnutls-bin/$(MEMO_PREFIX)$(MEMO_SUB_PREFIX) \
		$(BUILD_DIST)/libgnutls30/$(MEMO_LIBDIR) \
		$(BUILD_DIST)/libgnutlsxx28/$(MEMO_LIBDIR) \
		$(BUILD_DIST)/libgnutls28-dev/$(MEMO_LIBDIR)
	...
	cp -a $(BUILD_STAGE)/gnutls/$(MEMO_LIBDIR)/libgnutls.30.dylib $(BUILD_DIST)/libgnutls30/$(MEMO_LIBDIR)

	# gnutls.mk Prep libgnutlsxx28
	cp -a $(BUILD_STAGE)/gnutls/$(MEMO_LIBDIR)/libgnutlsxx.28.dylib $(BUILD_DIST)/libgnutlsxx28/$(MEMO_LIBDIR)

	# gnutls.mk Prep libgnutls28-dev
	cp -a $(BUILD_STAGE)/gnutls/$(MEMO_LIBDIR)/{pkgconfig,libgnutls{,xx}.dylib} $(BUILD_DIST)/libgnutls28-dev/$(MEMO_LIBDIR)
