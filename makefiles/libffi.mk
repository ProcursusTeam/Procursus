...
libffi-package: libffi-stage
	...
	mkdir -p $(BUILD_DIST)/libffi{8,-dev}/$(MEMO_LIBDIR)

	# libffi.mk Prep libffi8
	cp -a $(BUILD_STAGE)/libffi/$(MEMO_LIBDIR)/libffi.8.dylib $(BUILD_DIST)/libffi8/$(MEMO_LIBDIR)

	# libffi.mk Prep libffi-dev
	cp -a $(BUILD_STAGE)/libffi/$(MEMO_LIBDIR)/!(libffi.8.dylib) $(BUILD_DIST)/libffi-dev/$(MEMO_LIBDIR)
