...
cacerts-package: cacerts-stage
	...
	mkdir -p $(BUILD_DIST)/ca-certificates/$(MEMO_ALL_LIBDIR)/ssl

	# cacerts.mk Prep ca-certificates
	cp -a $(BUILD_STAGE)/cacerts/$(MEMO_PREFIX)/etc $(BUILD_DIST)/ca-certificates/$(MEMO_PREFIX)
	ln -s $(MEMO_PREFIX)/etc/ssl/certs $(BUILD_DIST)/ca-certificates/$(MEMO_ALL_LIBDIR)/ssl
	ln -s $(MEMO_PREFIX)/etc/ssl/certs/cacert.pem $(BUILD_DIST)/ca-certificates/$(MEMO_ALL_LIBDIR)/ssl
