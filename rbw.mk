...
rbw: rbw-setup pinentry
	...
	cd $(BUILD_WORK)/rbw && SDKROOT="$(TARGET_SYSROOT)" cargo \
		...
		--release \
		--target=$(RUST_TARGET)
	$(GINSTALL) -Dm755 $(BUILD_WORK)/rbw/target/$(RUST_TARGET)/release/rbw $(BUILD_STAGE)/rbw/$(MEMO_PREFIX)/$(MEMO_SUB_PREFIX)/rbw
	$(GINSTALL) -Dm755 $(BUILD_WORK)/rbw/target/$(RUST_TARGET)/release/rbw-agent $(BUILD_STAGE)/rbw/$(MEMO_PREFIX)/$(MEMO_SUB_PREFIX)/rbw-agent
	$(GINSTALL) -Dm755 $(BUILD_WORK)/rbw/bin/rbw-fzf $(BUILD_STAGE)/rbw/$(MEMO_PREFIX)/$(MEMO_SUB_PREFIX)/rbw-fzf
	$(GINSTALL) -Dm755 $(BUILD_WORK)/rbw/bin/pass-import $(BUILD_STAGE)/rbw/$(MEMO_PREFIX)/$(MEMO_SUB_PREFIX)/pass-import
