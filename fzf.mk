...
fzf-setup: setup
	...
	mkdir -p $(BUILD_STAGE)/fzf/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/share
...
fzf: fzf-setup
	+$(MAKE) -C $(BUILD_WORK)/fzf bin/fzf \
		...
		CGO_ENABLED=1 \
		CC="$(CC)"
	$(CP) -a $(BUILD_WORK)/fzf/bin $(BUILD_STAGE)/fzf/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)
	$(CP) -a $(BUILD_WORK)/fzf/man $(BUILD_STAGE)/fzf/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/share
