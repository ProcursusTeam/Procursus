...
libpixman-package: libpixman-stage
	...
	rm -rf $(BUILD_DIST)/libpixman{-dev}
	mkdir -p $(BUILD_DIST)/libpixman/usr/lib \
		$(BUILD_DIST)/libpixman-dev/usr/{lib,include/pixman-1}
	
	# libpixman.mk Prep libpixman
	cp -a $(BUILD_STAGE)/libpixman/usr/lib/libpixman-1.0.40.0.dylib $(BUILD_DIST)/libpixman/usr/lib

	# libpixman.mk Prep libpixman-dev
	cp -a $(BUILD_STAGE)/libpixman/usr/lib/{pkgconfig,libpixman-1.{a,dylib}} $(BUILD_DIST)/libpixman-dev/usr/lib
	cp -a $(BUILD_STAGE)/libpixman/usr/include/pixman-1/ $(BUILD_DIST)/libpixman-dev/usr/include/pixman-1
	
	# libpixman.mk Sign
	$(call SIGN,libpixman,general.xml)
	$(call SIGN,libpixman-dev,general.xml)
	
	# libpixman.mk Make .debs
	$(call PACK,libpixman,DEB_LIBPIXMAN_V)
	$(call PACK,libpixman-dev,DEB_LIBPIXMAN_V)
	
	# libpixman.mk Build cleanup
	rm -rf $(BUILD_DIST)/libpixman \
		$(BUILD_DIST)/libpixman-dev
