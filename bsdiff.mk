...
bsdiff: bsdiff-setup
	...
	mkdir -p $(BUILD_STAGE)/bsdiff/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/{bin,share/man/man1}

	+$(MAKE) -C $(BUILD_WORK)/bsdiff install \
		PREFIX="$(BUILD_STAGE)/bsdiff/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)" \
		INSTALL=install
...
bsdiff-package: bsdiff-stage
	...
	mkdir -p $(BUILD_DIST)/bsdiff/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)

	# bsdiff.mk Prep bsdiff
	cp -a $(BUILD_STAGE)/bsdiff/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/{bin,share} $(BUILD_DIST)/bsdiff/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)
