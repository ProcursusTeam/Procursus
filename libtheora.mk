...
libtheora-package: libtheora-stage
	...
	mkdir -p $(BUILD_DIST)/libtheora{0,-dev}/usr/lib

	# libtheora.mk Prep libtheora0
	cp -a $(BUILD_STAGE)/libtheora/usr/lib/libtheora{.0,dec.1,enc.1}.dylib $(BUILD_DIST)/libtheora0/usr/lib

	# libtheora.mk Prep libtheora-dev
	cp -a $(BUILD_STAGE)/libtheora/usr/lib/libtheora{,dec,enc}.{dylib,a} $(BUILD_DIST)/libtheora-dev/usr/lib
	cp -a $(BUILD_STAGE)/libtheora/usr/include $(BUILD_DIST)/libtheora-dev/usr
