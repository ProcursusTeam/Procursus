...
system-cmds: system-cmds-setup
	...
	cd $(BUILD_WORK)/system-cmds && $(CC) $(MEMO_ARCH) -isysroot $(TARGET_SYSROOT) $(PLATFORM_VERSION_MIN) -o passwd passwd.tproj/*.c -isystem include $(BUILD_BASE)/usr/lib/libcrypt.dylib
	cd $(BUILD_WORK)/system-cmds && $(CC) $(MEMO_ARCH) -isysroot $(TARGET_SYSROOT) $(PLATFORM_VERSION_MIN) -o dmesg dmesg.tproj/*.c -isystem include 
	cd $(BUILD_WORK)/system-cmds && $(CC) $(MEMO_ARCH) -isysroot $(TARGET_SYSROOT) $(PLATFORM_VERSION_MIN) -o sysctl sysctl.tproj/sysctl.c -isystem include 
	cd $(BUILD_WORK)/system-cmds && $(CC) $(MEMO_ARCH) -isysroot $(TARGET_SYSROOT) $(PLATFORM_VERSION_MIN) -o arch arch.tproj/*.c -isystem include -framework CoreFoundation -framework Foundation -lobjc 
	...
	for tproj in ac accton chpass dynamic_pager getconf getty hostinfo iostat login mkfile pwd_mkdb reboot sync vifs vipw zdump zic nologin; do \
		...
		echo "$$tproj" ; \
		$(CC) $(MEMO_ARCH) -isysroot $(TARGET_SYSROOT) $(PLATFORM_VERSION_MIN) -o $$tproj $$tproj.tproj/*.c -isystem include -D'__FBSDID(x)=' $$CFLAGS -F$(BUILD_BASE)/System/Library/Frameworks -framework CoreFoundation -framework IOKit $$LDFLAGS; \
	done
