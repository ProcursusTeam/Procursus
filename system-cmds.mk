...
DEB_SYSTEM-CMDS_V   ?= $(SYSTEM-CMDS_VERSION)-6
...
system-cmds-setup: setup libxcrypt
	...
	cp -a $(MACOSX_SYSROOT)/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/include/{libkern,net,servers,xpc} $(BUILD_WORK)/system-cmds/include
	cp -a $(MACOSX_SYSROOT)/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/include/{lib{c,proc},NSSystemDirectories,bootstrap,tzfile}.h $(BUILD_WORK)/system-cmds/include
	cp -a $(MACOSX_SYSROOT)/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/include/sys/{reboot,proc*,kern_control}.h $(BUILD_WORK)/system-cmds/include/sys
	cp -a $(MACOSX_SYSROOT)/System/Library/Frameworks/IOKit.framework/Headers/* $(BUILD_WORK)/system-cmds/include/IOKit
	cp -a $(BUILD_BASE)/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/include/{unistd,stdlib}.h $(BUILD_WORK)/system-cmds/include
	cp -a $(BUILD_BASE)/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/include/mach/{task,mach_host}.h $(BUILD_WORK)/system-cmds/include/mach
	cp -a $(BUILD_BASE)/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/include/crypt.h $(BUILD_WORK)/system-cmds/include	
	...
	wget -q -nc -P $(BUILD_WORK)/system-cmds/chpass.tproj \
		...
		https://raw.githubusercontent.com/coolstar/freebsd-ports-ios/master/$(MEMO_PREFIX)$(MEMO_SUBPREFIX).bin/chpass/{chpass{.h,.c,.1},edit.c,field.c,table.c,util.c}
...
system-cmds: system-cmds-setup
	...
	cd $(BUILD_WORK)/system-cmds && $(CC) -arch $(MEMO_ARCH) -isysroot $(TARGET_SYSROOT) $(PLATFORM_VERSION_MIN) -o passwd passwd.tproj/*.c -isystem include $(BUILD_BASE)/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/lib/libcrypt.dylib
	...
	for tproj in ac accton chpass dynamic_pager getconf getty hostinfo iostat login mkfile pwd_mkdb reboot sync vifs vipw zdump zic nologin; do \
		...
		EXTRA=; \
		case $$tproj in \
			chpass) CFLAGS="-Ichpass.tproj" LDFLAGS="$(BUILD_BASE)/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/lib/libcrypt.dylib";; \
			login) LDFLAGS="$(BUILD_BASE)/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/lib/libcrypt.dylib";; \
	...
	mkdir -p $(BUILD_STAGE)/system-cmds/{/bin,/sbin,/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/bin,/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/sbin,/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/share/man/man{1,5,8}}
	
	cp -a $(BUILD_WORK)/system-cmds/{reboot,nologin} $(BUILD_STAGE)/system-cmds/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/sbin
	cp -a $(BUILD_WORK)/system-cmds/pagesize.tproj/pagesize.sh $(BUILD_STAGE)/system-cmds/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/bin/pagesize
	...
	$(LN) -sf ../$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/sbin/reboot $(BUILD_STAGE)/system-cmds/sbin/halt
	cp -a $(BUILD_WORK)/system-cmds/{arch,chpass,getconf,getty,hostinfo,login,passwd} $(BUILD_STAGE)/system-cmds/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/bin
	$(LN) -sf chpass $(BUILD_STAGE)/system-cmds/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/bin/chfn
	$(LN) -sf chpass $(BUILD_STAGE)/system-cmds/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/bin/chsh
	cp -a $(BUILD_WORK)/system-cmds/{ac,accton,iostat,mkfile,pwd_mkdb,sysctl,vifs,vipw,zdump,zic} $(BUILD_STAGE)/system-cmds/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/sbin
	+	cp -a $(BUILD_WORK)/system-cmds/{arch,chpass,getconf,login,passwd}.tproj/*.1 $(BUILD_STAGE)/system-cmds/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/share/man/man1/
	cp -a $(BUILD_WORK)/system-cmds/{getty,nologin,sysctl}.tproj/*.5 $(BUILD_STAGE)/system-cmds/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/share/man/man5/
	cp -a $(BUILD_WORK)/system-cmds/{ac,accton,dmesg,dynamic_pager,getty,hostinfo,iostat,mkfile,nologin,pwd_mkdb,reboot,sync,sysctl,vifs,vipw,zdump,zic}.tproj/*.8 $(BUILD_STAGE)/system-cmds/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/share/man/man8/
	$(LN) -sf reboot.8.zst $(BUILD_STAGE)/system-cmds/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/share/man/man8/halt.8.zst
	$(LN) -sf chpass.1.zst $(BUILD_STAGE)/system-cmds/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/share/man/man1/chfn.1.zst
	$(LN) -sf chpass.1.zst $(BUILD_STAGE)/system-cmds/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/share/man/man1/chsh.1.zst
...
system-cmds-package: system-cmds-stage
	...
	$(FAKEROOT) chmod u+s $(BUILD_DIST)/system-cmds/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/bin/{passwd,login}
	$(FAKEROOT) chmod a+x $(BUILD_DIST)/system-cmds/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/bin/pagesize
	$(FAKEROOT) chmod 4555 $(BUILD_DIST)/system-cmds/$(MEMO_PREFIX)$(MEMO_SUBPREFIX)/bin/chpass
